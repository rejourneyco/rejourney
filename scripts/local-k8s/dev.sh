...
start_host_services() {
    ...
    start_process "worker-ingest" "" "cd '$ROOT_DIR/backend' && set -a && source '$ENV_FILE' && set +a && node --import tsx src/worker/ingestWorker.ts"
