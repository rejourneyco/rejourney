...
if [ "$DEPLOYMENT_TYPE" = "prod" ]; then
    if [ -n "${VITE_GOOGLE_ADS_CONVERSION_ID:-}" ]; then
        APP_SECRET_ARGS+=(--from-literal=VITE_GOOGLE_ADS_CONVERSION_ID="$VITE_GOOGLE_ADS_CONVERSION_ID")
    fi
    if [ -n "${VITE_GOOGLE_ADS_SIGNUP_CONVERSION_LABEL:-}" ]; then
        APP_SECRET_ARGS+=(--from-literal=VITE_GOOGLE_ADS_SIGNUP_CONVERSION_LABEL="$VITE_GOOGLE_ADS_SIGNUP_CONVERSION_LABEL")
    fi
fi
