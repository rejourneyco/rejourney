...
public final class VisualCapture: NSObject {
    ...
    @objc public func resumeFromUser() {
        guard _userPaused else { return }
        _userPaused = false
    ...
    @objc public func flushToDiskForCrash() {
...
private final class RedactionMask {
    ...
    private func _scanForSensitiveViews(in view: UIView, views: inout [WeakRegionRef], depth: Int = 0) {
        ...
        if SpecialCases.shared.isMapView(view) {
            // Descendant annotation coordinates move independently from the
            // map root. If one is explicitly marked, mask the stable map root
            // so the requested privacy boundary cannot drift away from content.
            if view.subviews.contains(where: { _containsExplicitMaskMarker(in: $0, depth: depth + 1) }) {
                views.append(WeakRegionRef(view: view, kind: .generic))
            }
            return
        }
    ...
    private func _scanForMediaViews(in view: UIView, regions: inout [RedactionRegion], depth: Int = 0) {
        ...
        // rejourney_occlude, which _maskKind handles on the sensitive pass.
        if SpecialCases.shared.isMapView(view) {
            return
        }
