...
public struct RejourneyOptions: Sendable, Equatable {
    ...
    public var maxSessionDurationMs: Int?
    public var detectRageTaps: Bool
    public var rageTapThreshold: Int
    public var rageTapTimeWindowMs: Int
    ...
    public init(
        ...
        captureFPS: Int? = nil,
        maxSessionDurationMs: Int? = nil,
        detectRageTaps: Bool = true,
        rageTapThreshold: Int = 3,
        rageTapTimeWindowMs: Int = 500,
        ...
        self.rageTapTimeWindowMs = max(1, rageTapTimeWindowMs)
...
final class RejourneyNativeController: NSObject {
    ...
    func start() async -> RejourneyStartResult {
        ...
        ReplayOrchestrator.shared.setRemoteConfig(
            ...
            maxRecordingMinutes: effectiveMaxRecordingMinutes(remote: effectiveRemoteConfig.maxRecordingMinutes)
        )
    ...
    func markVisualChange(_ importance: String) -> Bool {
        guard userPause == nil else { return false }
    ...
    func onScroll() {
        guard userPause == nil else { return }
    ...
    private func effectiveMaxRecordingMinutes(remote: Int) -> Int {
        guard let milliseconds = options.maxSessionDurationMs, milliseconds > 0 else {
            return remote
        }
        let localMinutes = max(1, Int(ceil(Double(milliseconds) / 60_000.0)))
        return min(remote, localMinutes)
    }
}
...
struct RejourneyCaptureSettings: Equatable {
    ...
    init(
        ...
        var settings: [String: Any] = [
            ...
            "rageTapTimeWindow": options.rageTapTimeWindowMs,
