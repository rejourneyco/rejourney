...
final class VisualCapture: NSObject {
    ...
    private var _externalRedactionRects: [String: CGRect] = [:]
    private let _externalRedactionLock = NSLock()
    ...
    func setExternalRedactionRect(id: String, rect: CGRect) {
        guard !id.isEmpty else { return }
        _externalRedactionLock.lock()
        _externalRedactionRects[id] = rect
        _externalRedactionLock.unlock()
    }

    func removeExternalRedactionRect(id: String) {
        _externalRedactionLock.lock()
        _externalRedactionRects.removeValue(forKey: id)
        _externalRedactionLock.unlock()
    }

    private func _externalRedactionRegions() -> [RedactionRegion] {
        _externalRedactionLock.lock()
        let rects = Array(_externalRedactionRects.values)
        _externalRedactionLock.unlock()
        return rects.map { RedactionRegion(rect: $0, kind: .generic) }
    }
    ...
    private func _captureFrame(forced: Bool = false) {
        ...
        autoreleasepool {
            ...
            let redactionRegions = _redactionMask.computeRegions(windows: captureWindows) + _externalRedactionRegions()
